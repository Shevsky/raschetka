...
export const NotGroupedItemsSection = observer(() => {
  const store = useContext(CheckContext);

  const items = store.notGroupedItems;
  const itemsSum = store.notGroupedItemsSum;
  ...
  return (
    <LabeledRow name="Товары" counter={items.length} description={formatMoney(itemsSum)}>
      {items.map((item) => (
